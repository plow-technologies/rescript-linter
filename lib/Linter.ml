...
let printError ppf src ({loc; msg} : Location.error) =
  ...
  (* Format.fprintf ppf "@[<v>@, %a@, %s@,@]" (print ~message_kind:`error
     "We've found a bug for you!") src loc msg; *)
  Format.fprintf ppf "@[<v>@,  %a@,  %s@,@]"
    (Res_diagnostics_printing_utils.Super_location.print ~message_kind:`error
       "Lint error!" src )
    loc msg

let printError src msg d =
  printError Format.err_formatter src
    Location.
